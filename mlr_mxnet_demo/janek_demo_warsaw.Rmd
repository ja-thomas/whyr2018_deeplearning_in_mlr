...
* clone branch `mxnet` from [https://github.com/mlr-org/mlr](https://github.com/mlr-org/mlr)
* build the package to `tar.gz` file by navigating in repo parent and running `R CMD build mlr` in terminal
* install the package in R via `ìnstall.packages("<path to pkg>/mlr_2.12.tar.gz", repo = NULL, type = "source")` and verify via `sessionInfo()`
* reinstall up to date mlr version
* source("raw github learner code")
* for mxnet installation follow instructions on [the mxnet page](https://mxnet.incubator.apache.org/install/index.html)
* packages required for this demo are __mxnet, mlr, mlrMBO, mlbench__ and __farff__
...
* define MBO as search algorithm using mlrMBO.
